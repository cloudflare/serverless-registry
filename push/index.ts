...
await withPushWorkspace(imageID.trim(), async ({ tarFile, imagePath, cacheFolder }) => {
  if (!(await file(tarFile).exists())) {
    const output = await $`docker save ${image} --output ${tarFile}`;

    if (output.exitCode != 0) {
      throw new Error(`Error saving image ${image}: ${output.text()}`);
    }
  ...
  if (manifests.length == 0) {
    throw new Error("unexpected manifest of length 0");
  }
  ...
  async function pushLayer(layerDigest: string, readableStream: ReadableStream, totalLayerSize: number) {
    ...
    let location =
      createUploadResponse.headers.get("location") ?? `/v2${imageRepositoryPath}/blobs/uploads/${uploadId}`;
